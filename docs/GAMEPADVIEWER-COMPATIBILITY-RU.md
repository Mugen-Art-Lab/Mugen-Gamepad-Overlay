...
Mugen Gamepad Overlay не запускает страницу GamepadViewer и не является браузером. Плагин читает локальный CSS как описание слоёв, загружает локальные SVG/PNG/JPEG и отрисовывает их собственным движком внутри OBS.
...
![Двуязычная плашка ошибки загрузки скина](images/bilingual-error.png)
