...
В репозитории хранится компактная часть проекта, относящаяся непосредственно к плагину. `BUILD_WINDOWS.cmd` создаёт игнорируемую рабочую папку на основе зафиксированной ревизии официального шаблона OBS, собирает конфигурацию Windows x64 Release и подготавливает установщик и ZIP-архивы. Ручной workflow **Windows Release Build** в GitHub Actions умеет выполнить такую же чистую релизную сборку удалённо и выдать временный artifact, не публикуя GitHub Release автоматически.
...
Плагин считывает ввод локально. В нём нет телеметрии, рекламы, браузерного движка и автоматических или фоновых сетевых запросов. Кнопки проекта и Mugen Art Lab в разделе **«О проекте»** открывают GitHub в системном браузере только после нажатия пользователем.
